...
//        velocityVolumes[j][i].connect(masterGain);
//    }
//}
...
function startNote(note, velocity) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        oscillator.type = oscTypes[i];
        oscillator.frequency.value = allFrequencies[note];
        if (i === 1) {oscillator.detune.value = 2; }
        ...
        oscillator.connect(velocityVolumes[i][note]);
        oscillator.start();

        envelopeOn(velocityVolumes[i][note], velocity);

        oscillators[i][note] = oscillator;
    }
}

function stopNote(note, velocity) {
    for (let i = 0; i < 2; i++) {
        envelopeOff(velocityVolumes[i][note], velocity);
